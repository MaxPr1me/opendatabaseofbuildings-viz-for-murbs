...
### Added — NBC Part 3 classification, data store, and downstream outputs

- **Data-derived type normalization** (`config/type_normalization.yaml`) generated from the
  full-population schema audit (English + French/Québec); replaces the hard-coded English map
- **NBC Part 3 classification** — storey/unit corroboration required; low-rise/single-family
  excluded; thresholds moved to `config/default.yaml`
- **MURB-subset datastore** (`src/murb_geometry/datastore.py`) — persists precision/tiered
  subsets with provenance manifests; `murb-geometry data-status` reports validity
- **Visualization module** (`src/murb_geometry/visualization/charts.py`) + `figures` CLI —
  publication figures from persisted outputs
- **Reporting module** (`src/murb_geometry/reporting/summary.py`) — per-province coverage/quality
  report (`coverage_report.csv`), auto-generated by the pipeline
- **`excel-audit` CLI command** — building-level audit workbook from the MURB subsets
- **gbXML XSD validation** (`validate_gbxml`, `validate_gbxml_against_xsd`) — optional lxml-based
  schema validation with graceful fallback
- **CRS enforcement guard** (`ensure_projected_crs`) at the data-loading boundary
- **Type-mapping worksheet/proposal scripts** (`scripts/build_type_mapping_worksheet.py`,
  `scripts/propose_type_normalization.py`)

### Changed — this iteration

- Classifier loads the type mapping from config; rules realigned to NBC Part 3
- `compute_metrics_vectorized` genuinely vectorizes area/perimeter/compactness/convexity
  (MRR/holes/vertices per geometry); results verified identical to the per-row reference
- Research report (`scripts/generate_research_report.py`) prose is now accurate and dynamic
- Figures rebuilt from the MURB subsets (removed stale prototype figure generator)
- `docs/limitations.md` documents the classification coverage bias (attribute sparsity)

### Fixed — this iteration

- Quebec (and other French/low-attribute provinces) no longer silently dropped — French `type`
  values are recognized; unclassifiable records surface as `insufficient_information`
- Removed dead code in `geometry/metrics.vertex_count`
